...
const YESTERDAY_ISO = new Date(
  new Date(`${TODAY_ISO}T00:00:00Z`).getTime() - 86_400_000,
)
  .toISOString()
  .slice(0, 10)
...
const elapsedDays: DayRecord[] = allDays.filter(
  (day) => day.isoDate <= TODAY_ISO,
)
...
export function getTrendSummary(rangeId: TrendRangeId) {
  ...
  const previousAverage = previousWindow.length
    ? previousWindow.reduce((total, point) => total + point.score, 0) /
      previousWindow.length
    : current

  // Walk back from today so the most recent day is always plotted.
  const sampled: typeof window = []
  for (let index = window.length - 1; index >= 0; index -= range.step)
    sampled.unshift(window[index])
  ...
  return {
    ...
    points: sampled.map((point) => ({
      ...
      score: point.score,
      label: range.useWeekdayLabels
        ? weekdayLabel(point.isoDate)
        : formatShortDate(point.isoDate),
...
export function getParameterHighlights() {
  ...
  const byChange = [...parameters].sort(
...
export function getActivitySummary() {
  ...
  const totalMinutes = activeDayRecords.reduce(
    (total, day) => total + (day.session?.sessionMinutes ?? 0),
    0,
  )
  const ratio = activeDays / ACTIVITY_WINDOW
  const level: ActivityLevelId =
    ratio >= 0.7
      ? "high"
      : ratio >= 0.4
        ? "moderate"
  ...
  return {
    ...
    sessions: activeDays,
    totalMinutes,
    averageSessionMinutes: activeDays
      ? Math.round(totalMinutes / activeDays)
      : 0,
    activitiesThisWeek: window.reduce(
      (total, day) => total + (day.session?.gamesCompleted ?? 0),
      0,
    ),
    activitiesLast30Days: last30.reduce(
      (total, day) => total + (day.session?.gamesCompleted ?? 0),
      0,
    ),
    ratio,
...
export function getLastSession() {
  ...
  return {
    ...latest,
    lastActiveLabel: `${latest.dayLabel}, ${latest.timeLabel}`,
  }
}
...
export function getAttentionState() {
  ...
  if (!declining || Math.abs(declining.changePercent) < WATCH_DECLINE) {
    return {
      ...
      message:
        "Cognitive performance and activity levels are stable compared with the previous week.",
  ...
  const severity: AttentionSeverity =
    drop >= SUSTAINED_DECLINE ||
    (drop >= SIGNIFICANT_DECLINE && reducedActivity)
      ? "attention"
      : "monitor"
...
export function getMonitoringStatus() {
  ...
  if (
    ...
    reducedActivity ||
    (attention.severity === "monitor" &&
      (week.changePercent < 0 || activity.level !== "high"))
  ) {
    id = "monitoring"
  }
...
export function getGamePerformance() {
  ...
  for (const game of games) {
    ...
    focusTotals.set(game.focus, {
      total: entry.total + game.score,
      count: entry.count + 1,
    })
  }
...
export function getCurrentMonthIndex(): number {
  const index = dashboardData.calendarMonths.findIndex((month) =>
