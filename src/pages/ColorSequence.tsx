...
const COLORS: ColorItem[] = [
  { id: "red", bg: "bg-red-500", activeBg: "bg-red-300", border: "border-red-600", labelKey: "games.colorSequence.red" },
  { id: "blue", bg: "bg-blue-500", activeBg: "bg-blue-300", border: "border-blue-600", labelKey: "games.colorSequence.blue" },
  { id: "green", bg: "bg-green-500", activeBg: "bg-green-300", border: "border-green-600", labelKey: "games.colorSequence.green" },
  { id: "yellow", bg: "bg-yellow-400", activeBg: "bg-yellow-200", border: "border-yellow-500", labelKey: "games.colorSequence.yellow" },
];
...
export function ColorSequence() {
  ...
  const handleColorPress = (colorId: string) => {
    ...
    setTimeout(() => setUserFlash(null), 250);

    const nextUserSeq = [...userSeq, colorId];
    const currentIndex = nextUserSeq.length - 1;

    if (colorId !== gameSeq[currentIndex]) {
    ...
    if (nextUserSeq.length === gameSeq.length) {
      setGameState("correct");
      if (level > highScore) {
        setHighScore(level);
      }
      timeoutRef.current = setTimeout(() => {
        nextLevel(level, gameSeq);
      }, 1000);
    }
  };
      ...
      <div className="space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col items-center text-center py-8 md:py-12">
          ...
          <p className="text-slate-500 font-bold text-[20px] sm:text-base mt-2 max-w-xs">
            ...
            <span className="font-extrabold text-[#9333EA]">{t("games.colorSequence.level", { level })}</span>!
          </p>
          ...
          <div className="mt-6 w-32 h-32 md:w-36 md:h-36 rounded-full border-[6px] border-purple-200 flex flex-col items-center justify-center bg-white shadow-md">
            ...
            <span className="text-[18px] font-bold text-slate-400 uppercase tracking-wide mt-0.5">
              {t("games.colorSequence.level", { level: "" }).trim()}
            </span>
          </div>
          ...
          {highScore > 0 && (
            <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2">
              <p className="text-[20px] font-bold text-amber-800">
                🏆 {t("games.colorSequence.bestScore", { level: highScore })}
              </p>
            </div>
          )}
      ...
      <div className="bg-white border-2 border-slate-100 rounded-3xl shadow-md p-5 sm:p-6 md:p-8">
        ...
        {gameState === "gameover" && (
          <div className="flex items-center justify-center gap-3 mt-6 animate-in fade-in duration-200">
            ...
            >
              <RotateCcw size={18} />
              {t("games.colorSequence.tryAgain")}
            </button>
            <button
              onClick={handleGameOver}
              className="px-6 py-3 bg-slate-100 text-slate-700 font-bold rounded-xl hover:bg-slate-200 transition-all shadow-sm cursor-pointer min-h-[48px]"
            >
              {t("games.colorSequence.seeResults")}
            </button>
          </div>
        )}
      </div>
