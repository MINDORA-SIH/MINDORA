...
export function SpotTheDifference() {
  ...
  const remaining = differences.filter(
    (difference) => !found.includes(difference.id),
  )
    ...
    <div className="space-y-5 md:space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="shrink-0 bg-[#F5F0FF] border-2 border-[#EBE0FF] rounded-full px-3 sm:px-4 py-1.5 flex items-center gap-1.5">
          ...
          <span className="font-extrabold text-[18px] sm:text-[20px] text-[#1E2445]">
            {found.length}/3
          </span>
        </div>
      </div>
      ...
      <div className="bg-white border-2 border-slate-100 rounded-3xl shadow-md p-5 sm:p-6 md:p-8">
          ...
          {complete ? (
            <div className="space-y-1 animate-in fade-in duration-200">
              ...
              <p className="text-slate-500 font-bold">
                {t("games.spotTheDifference.foundEvery")}
              </p>
            </div>
          ) : (
            <p className="text-slate-500 font-bold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
          {(["left", "right"] as const).map((side) => (
            <section
              key={side}
              className="rounded-3xl border-2 border-[#D9D4F5] bg-[#F9F8FF] p-5 min-h-[245px]"
            >
              <h2 className="text-center font-extrabold text-[#534AB7] mb-4">
                {t("games.spotTheDifference.picture", {
              ...
              <div className="grid grid-cols-3 gap-3 place-items-center text-4xl sm:text-5xl">
                ...
                <button
                  ...
                  className={`rounded-2xl p-2 transition ${
                    found.includes("sun")
                      ...
                      : "hover:bg-white focus-visible:ring-2 focus-visible:ring-[#534AB7]"
                  }`}
                  aria-label={`Find the sun difference in picture ${
                    side === "left" ? "A" : "B"
                  }`}
                >
                ...
                <button
                  ...
                  className={`rounded-2xl p-2 transition ${
                    found.includes("flower")
                      ? "bg-emerald-100 ring-2 ring-emerald-400"
                      : "hover:bg-white focus-visible:ring-2 focus-visible:ring-[#534AB7]"
                  }`}
                  aria-label={`Find the flower difference in picture ${
                    side === "left" ? "A" : "B"
                  }`}
                >
                ...
                <button
                  ...
                  aria-label={`Find the bird difference in picture ${
                    side === "left" ? "A" : "B"
                  }`}
                >
                  {side === "left" ? (
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-center gap-3">
          <p className="text-slate-500 font-bold text-center">
            {remaining.length
              ? t("games.spotTheDifference.differencesLeft", {
                  count: remaining.length,
                })
              : t("games.spotTheDifference.allFound")}
          </p>
          <button
            onClick={restart}
