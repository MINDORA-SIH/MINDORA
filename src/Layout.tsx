...
import { LanguageDropdown, LANGUAGES, DEFAULT_LANGUAGE, type Language } from "./components/LanguageDropdown";
...
export function Layout() {
  ...
  const [language, setLanguage] = useState<Language>(() => LANGUAGES.find((item) => item.code === i18n.language) ?? DEFAULT_LANGUAGE);
  ...
  const speechRecognitionLanguage = getLangConfig(language.code).speechRecognitionLang;
  ...
  const handleVoiceCommand = (text: string) => {
    ...
    if (lower.includes("game") || lower.includes("home") || lower.includes("play")) {
      ...
      setVoiceFeedback(t("layout.heard", { text, lang: speechRecognitionLanguage }));
    }
  };
