...
/**
 * Speech recognition tracks the selected UI locale where a native browser voice
 * is available; unsupported locales use the safe English-India fallback.
 */
export function Layout() {
  ...
  const [language, setLanguage] = useState<Language>(
    () =>
      LANGUAGES.find((item) => item.code === i18n.language) ?? DEFAULT_LANGUAGE,
  )

  const [isListening, setIsListening] = useState(false)
  const [darkMode, setDarkMode] = useState(
    () => localStorage.getItem("mindora_dark") === "true",
  )
  ...
  const speechRecognitionLanguage = getLangConfig(
    language.code,
  ...
  useEffect(() => {
    ...
    setSlideDirection(
      currIndex >= prevIndex ? "page-slide-right" : "page-slide-left",
    )
  ...
  const handleVoiceCommand = (text: string) => {
    ...
    if (
      ...
      setVoiceFeedback(
        t("layout.heard", { text, lang: speechRecognitionLanguage }),
      )
    }
  }
  ...
  return (
    <div
      className="w-full min-h-screen flex flex-col relative transition-colors duration-300"
    ...
    >
        ...
        <div className="flex items-center gap-2 sm:gap-3 md:gap-4 min-w-0">
          ...
          <div className="flex flex-col gap-0.5 sm:gap-1 min-w-0">
            <NavLink to="/" className="hover:opacity-90 transition-opacity">
              <span
                className="font-extrabold text-lg sm:text-xl md:text-2xl tracking-tight leading-none"
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
          ...
          {isWide && (
            <button
              ...
              title={
                darkMode ? t("layout.switchToLight") : t("layout.switchToDark")
              }
              className={clsx(
      ...
      {sosActive && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 transition-all bg-black/30 backdrop-blur-md">
          <div
            className={clsx(
          ...
          >
            {!sosSent ? (
              <>
                ...
                <div className="relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-5xl sm:text-6xl font-black text-red-600">
                      {sosCountdown}
                    </span>
                  </div>
                </div>
                ...
                <div>
                  <h3 className="font-extrabold text-xl sm:text-2xl text-red-700">
                    🚨 {t("layout.emergencyAlert")}
                  </h3>
              ...
              <>
                ...
                <div>
                  <h3 className="font-extrabold text-xl sm:text-2xl text-slate-700">
                    {t("layout.alertSent")}
                  </h3>
        ...
        <>
          ...
          <span
            className={clsx(
              ...
              isActive
                ? "text-[var(--dock-active)]"
                : "text-[var(--dock-inactive)]",
            )}
          >
            {label}
          </span>
