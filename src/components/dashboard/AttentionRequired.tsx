...
function Fact({
  ...
  return (
    <div
      className={clsx(
        "rounded-2xl border border-white/70 bg-white/75 px-3.5 py-2.5 dark:border-white/10 dark:bg-white/5",
        className,
      )}
    >
      <dt
      ...
      <dd
        className="mt-0.5 text-[16px] font-extrabold leading-snug"
...
export function AttentionRequired({
  ...
  return (
    <section
      aria-live="polite"
      className={clsx(
        "rounded-3xl border-2 p-5 shadow-sm sm:p-6",
        toneStyle.surface,
        toneStyle.border,
      )}
    >
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex min-w-0 items-start gap-3.5">
          ...
          <div className="min-w-0">
            <h2
              className={clsx(
                "text-[19px] font-extrabold leading-tight sm:text-xl",
                toneStyle.text,
              )}
            >
              {attention.title}
      ...
      {parameter ? (
        <dl className="mt-4 grid grid-cols-2 gap-2.5 sm:gap-3 lg:grid-cols-4">
          <Fact label={t("dashboard.currentScore", "Current score")} value={`${parameter.score} / 100`} />
          <Fact
            label={t("dashboard.previousWeek", "Previous week")}
            value={`${parameter.previousScore} / 100`}
          />
          <Fact label={t("dashboard.change", "Change")} value={`${parameter.changePercent}%`} />
          <Fact
            label={t("dashboard.lastRelatedActivity", "Last related activity")}
