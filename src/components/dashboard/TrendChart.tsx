...
export function TrendChart({
  ...
  const lower = Math.max(
    0,
    Math.floor((Math.min(...scores, previousAverage) - 3) / 5) * 5,
  )
  const upper = Math.min(
    100,
    Math.ceil((Math.max(...scores, previousAverage) + 3) / 5) * 5,
  )
  ...
  const yOf = (score: number) =>
    PAD_TOP + (1 - (score - lower) / domain) * plotHeight

  const line = points
    .map(
      (point, index) =>
        `${xOf(index).toFixed(1)},${yOf(point.score).toFixed(1)}`,
    )
    .join(" ")
  ...
  const shouldLabel = (index: number) =>
    index === lastIndex ||
    (index % labelEvery === 0 && lastIndex - index >= labelEvery)
  ...
  return (
    <div>
      <svg
        ...
        role="img"
        aria-label={t("dashboard.performanceIndexAria", "Performance Index over the selected period")}
      >
        {[0, 0.5, 1].map((step) => (
          <line
            key={step}
        ...
        {points.map((point, index) => (
          <g
            ...
            role="button"
            tabIndex={0}
            aria-label={`${point.fullLabel}: ${t("dashboard.performanceIndex", "Performance Index")} ${point.score}`}
                ...
                transform:
                  index === 0
                    ...
                    : isLast
                      ...
                      : "translateX(-50%)",
                color:
                  index === activeIndex ? "var(--foreground)" : "var(--muted)",
              }}
            >
              {point.label}
            </span>
