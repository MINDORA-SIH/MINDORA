...
const CELL_CLASSES: Record<CalendarDayStatus, string> = {
  ...
  future:
    "border-dashed border-[#E4E9F1] text-[#6B7788] dark:border-[#2C3A52] dark:text-[#8B98AC]",
}
...
export function ActivityCalendar() {
  ...
  const STATUS_TEXT: Record<CalendarDayStatus, string> = {
    completed: t("dashboard.statusCompleted", "Full session completed"),
    partial: t("dashboard.statusPartial", "Short activity only"),
    none: t("dashboard.statusNone", "No activity recorded"),
    future: t("dashboard.statusFuture", "Upcoming date"),
  }

  const LEGEND: { label: string; className: string }[] = [
    {
      label: t("dashboard.legendCompleted", "Completed"),
      className:
        "bg-[#DFF4E8] border-[#B6E3C8] dark:bg-[#183D2E] dark:border-[#27543E]",
    },
    {
      label: t("dashboard.legendPartial", "Partial"),
      className:
        "bg-[#FDF2DC] border-[#F0D79E] dark:bg-[#3D2E12] dark:border-[#5E4718]",
    },
    {
      label: t("dashboard.legendNoActivity", "No activity"),
      className:
        "bg-[#F4F6FA] border-[#DCE3EC] dark:bg-[#222D40] dark:border-[#33405A]",
    },
    {
      label: t("dashboard.legendUpcoming", "Upcoming"),
      className: "border-dashed border-[#E4E9F1] dark:border-[#2C3A52]",
    },
  ]
  ...
  const monthNav = (
    <div className="flex items-center gap-1.5">
      ...
      <span
        className="min-w-[8.5rem] text-center text-[15px] font-extrabold"
        style={{ color: "var(--foreground)" }}
      >
        {month.label}
      </span>
      <button
        type="button"
        onClick={() =>
          setMonthCursor((current) => Math.min(current + 1, months.length - 1))
        }
  ...
  return (
    <SectionCard
      title={t("dashboard.activityCalendarTitle", "Activity Calendar")}
      subtitle={t("dashboard.activityCalendarSubtitle", "Patient participation by day")}
      icon={CalendarDays}
      tone="info"
      action={monthNav}
      className="h-full"
    >
      <ul className="flex flex-wrap items-center gap-x-4 gap-y-2">
        {LEGEND.map((item) => (
          ...
          >
            <span
              aria-hidden="true"
              className={clsx("h-3.5 w-3.5 rounded-md border", item.className)}
            />
            {item.label}
          </li>
        ))}
        ...
        >
          <span
            aria-hidden="true"
            className="h-2.5 w-2.5 rounded-full bg-[#3B82C4]"
          />
        ...
        {month.days.map((day) => {
          ...
          if (day.status === "future") {
            return (
              <span
                key={day.isoDate}
                className={clsx(shared, "flex items-center justify-center")}
              >
                {day.day}
              </span>
            )
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                isSelected &&
                  "ring-2 ring-[#6C5CC4] ring-offset-2 ring-offset-[var(--card-bg)]",
              )}
            >
              {day.day}
              {day.isToday ? (
                <span
                  aria-hidden="true"
                  className="mt-0.5 h-1.5 w-1.5 rounded-full bg-[#3B82C4]"
                />
              ) : null}
            </button>
          )
      ...
      <div
        className="mt-4 rounded-2xl border-2 border-slate-200 p-4 dark:border-slate-700"
        aria-live="polite"
      >
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          ...
          >
            ...
            {selectedDay.isToday ? ` · ${t("dashboard.today", "Today")}` : ""}
          </h3>
          <p
            className="text-[14px] font-bold"
            style={{ color: "var(--muted)" }}
          >
            {weekdayLabel(selectedDay.isoDate)} ·{" "}
            {STATUS_TEXT[selectedDay.status]}
          </p>
        </div>
        ...
        {selectedDay.session ? (
          <dl className="mt-3 grid grid-cols-2 gap-2.5 sm:grid-cols-3">
            ...
            />
            <StatTile
              label={t("dashboard.sessionLengthLabel", "Session length")}
              value={`${selectedDay.session.sessionMinutes} ${t("dashboard.min", "min")}`}
            />
            <StatTile
              label={t("dashboard.performanceIndexLabel", "Performance Index")}
              value={`${selectedDay.session.performanceIndex} / 100`}
              tone="info"
            />
            <StatTile
              label={t("dashboard.strongestAreaLabel", "Strongest area")}
              value={selectedDay.session.strongestArea}
              tone="stable"
            />
            <StatTile
              label={t("dashboard.areaToMonitorLabel", "Area to monitor")}
              value={selectedDay.session.areaToMonitor}
              tone="monitor"
            />
          </dl>
        ) : (
          <p
            ...
            style={{ color: "var(--muted)" }}
          >
            {t("dashboard.noActivityRecordedDesc", "No activity was recorded on this date. Occasional rest days are expected.")}
          </p>
        )}
      </div>
    </SectionCard>
