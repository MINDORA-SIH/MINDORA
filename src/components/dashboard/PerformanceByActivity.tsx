 ...
 */
export function PerformanceByActivity({
  games,
  focusInsight,
}: PerformanceByActivityProps) {
  const { t } = useTranslation()
  const infoTone = TONES.info

  return (
    <SectionCard
      title={t("dashboard.performanceByActivityTitle", "Performance by Activity")}
      subtitle={t("dashboard.performanceByActivitySubtitle", "Current performance by activity, with change from the previous session set")}
      icon={Gauge}
      tone="info"
    >
          ...
          return (
            ...
            >
              ...
              <div className="mt-2.5 flex items-center justify-between gap-2">
                ...
                <TrendBadge
                  changePercent={game.changePercent}
              ...
              <div className="mt-2">
                <ProgressBar
                  ...
                  label={`${game.name}: ${game.score} ${t("dashboard.outOf100", "out of 100")}`}
                />
              </div>
            </li>
          )
      ...
      <p
        className={clsx(
          "mt-3 rounded-2xl border px-4 py-3 text-[15px] font-bold",
          infoTone.surface,
          infoTone.border,
        )}
        style={{ color: "var(--foreground)" }}
